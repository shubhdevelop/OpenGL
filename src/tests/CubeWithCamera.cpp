...
CUbeWithCamera::CUbeWithCamera()
    : Test(),
      ...
      m_indexes{// Front face
                ...
                // Left face
                20, 21, 22, 22, 23, 20},
      m_vb(m_positions, 24 * 8 * sizeof(float)), m_ib(m_indexes, 36),
      m_shader("resource/shaders/cubeWithCamera.shader.glsl"),
      m_texture("resource/water_texture.jpg"), m_renderer() {
...
void CUbeWithCamera::onUpdate(float deltaTime) { m_time += deltaTime; };

void CUbeWithCamera::onRender(glm::mat4 view, glm::mat4 projection) {
  GLCall(glEnable(GL_DEPTH_TEST));
  m_shader.Bind();
  m_texture.Bind(0);
...
void CUbeWithCamera::onImGuiRender() {
  ImGui::Begin("Model 1");
