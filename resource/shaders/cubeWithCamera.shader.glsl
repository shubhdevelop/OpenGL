...
void main()
{
    ...
    vec2 scaledCoord = v_TexCoord * 8.0;
    bool check = (mod(floor(scaledCoord.x) + floor(scaledCoord.y), 2.0) == 0.0);
    vec3 color1 = vec3(1.0, 1.0, 1.0);
    vec3 color2 = vec3(0.0, 0.0, 0.0);
    vec3 finalColor = check ? color1 : color2;
    color = vec4(finalColor * texColor.rgb, 1.0);
}
